    ...
    *,
    ...
    if recipe_in.submitter_id != current_user.id:
        raise HTTPException(
            ...
            detail=f"You can only submit recipes as yourself"
        )
...
@router.put("/", status_code=status.HTTP_201_CREATED, response_model=Recipe)
def update_recipe(
    *,
    ...
    if not recipe:
        raise HTTPException(
            ...
            detail=f"Recipe with ID: {recipe_in.id} not found."
        )

    # if recipe.submitter_id != current_user.id:
    #     raise HTTPException(
    #         status_code=status.HTTP_403_FORBIDDEN,
    #         detail=f"You can only update your recipes."
    #     )
